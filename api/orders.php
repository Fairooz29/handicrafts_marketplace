...
function checkAuth() {
    ...
    if (!isset($_SESSION['user_id']) || !isset($_SESSION['logged_in']) || $_SESSION['logged_in'] !== true) {
        // Try to get user ID from localStorage via request
        $headers = getallheaders();
        error_log("Request headers: " . print_r($headers, true));
        
        // Check if we have a user_id in the request
        ...
        // Fallback to localStorage check via JavaScript
        ...
        // If we still don't have a user ID, check if there's one in the JSON payload
        $input = json_decode(file_get_contents('php://input'), true);
        if (isset($input['user_id'])) {
            $userId = intval($input['user_id']);
            error_log("Using user ID from request body: " . $userId);
            return $userId;
        }
