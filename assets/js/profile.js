...
const ProfileManager = (function() {
    ...
    function populateProfileForm(userData) {
        ...
        if (userData.profile_image) {
            const cacheBustUrl = `${userData.profile_image}${userData.profile_image.includes('?') ? '&' : '?'}t=${Date.now()}`;
                ...
                <div class="order-items">
                    ${(order.items || []).map(item => `
                        <div class="order-item">
                            <img src="${item.image_url || item.image || 'assets/images/placeholder.jpg'}" alt="${item.name}" class="item-image">
    ...
    async function uploadAvatar(file) {
        ...
        try {
            ...
            if (data.success && data.data && data.data.profile_image) {
                const newUrl = data.data.profile_image;
                localStorage.setItem('profile_image', newUrl);

                // Update all avatar images on the page
                const cacheBustUrl = `${newUrl}${newUrl.includes('?') ? '&' : '?'}t=${Date.now()}`;
                document.querySelectorAll('.large-avatar, .profile-avatar').forEach(img => {
                    img.src = cacheBustUrl;
                });

                // Also update header avatar if exists
                const headerAvatar = document.querySelector('.profile-avatar-link img.profile-avatar');
                if (headerAvatar) headerAvatar.src = cacheBustUrl;

                showNotification('Photo updated successfully!', 'success');
            } else {
                showNotification((data && data.message) ? data.message : 'Failed to update photo', 'error');
            }
        } catch (err) {
    ...
    async function loadWishlist() {
        try {
            ...
            if (data.success) {
                const items = (data.data && Array.isArray(data.data.favorites)) ? data.data.favorites : [];
    ...
    function displayWishlist(items) {
        ...
        wishlistGrid.innerHTML = items.map(item => `
            <div class="wishlist-item">
                <img src="${item.image || item.image_url || 'assets/images/placeholder.jpg'}" alt="${item.name}" class="item-image">
                <div class="item-info">
                ...
                <div class="item-actions">
                    <button class="add-to-cart-btn" data-product-id="${item.id}" onclick="HandicraftsCart.addToCart(${item.id})">Add to Cart</button>
                    <button class="remove-wishlist-btn" onclick="HandicraftsFavorites.toggleFavorite(${item.id}).then(()=>ProfileManager.loadWishlist())">Remove</button>
                </div>
            </div>
        `).join('');
    }
