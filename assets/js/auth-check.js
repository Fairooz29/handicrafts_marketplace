...
function updateHeader(isLoggedIn) {
    ...
    if (isLoggedIn) {
        ...
        if (profileLink) {
            ...
            const img = profileLink.querySelector('img.profile-avatar');
            if (img && profileImage) {
                // Cache-bust to reflect latest image
                const cacheBust = `${profileImage}${profileImage.includes('?') ? '&' : '?'}t=${Date.now()}`;
                img.src = cacheBust;
            }
        } else {
            ...
            profileImg.src = profileImage ? profileImage : 'assets/images/profile.jpg';
